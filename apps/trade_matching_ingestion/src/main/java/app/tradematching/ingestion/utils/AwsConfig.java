...
import app.tradematching.ingestion.kcl.RecordProcessorFactory;
import app.tradematching.ingestion.kcl.SettlementRecordProcessorFactory;
...
public class AwsConfig {
    ...
    public DynamoDbTable<Settlement> getSettlementTable() throws DynamoDBConnectionException {
        ...
        try {
            ...
            table = ddbClient.table(awsProperties.getSettlementStateTableName(), TableSchema.fromImmutableClass(Settlement.class));
        } catch (Exception e) {
            log.error("Error Connecting to dynamoDB or getting Settlement Table", e);
    ...
    public DynamoDbTable<NackMessage> getNackTable() throws DynamoDBConnectionException {
        ...
        try {
            ...
            table = ddbClient.table(awsProperties.getStateTableName(), TableSchema.fromImmutableClass(NackMessage.class));
        } catch (Exception e) {
    ...
    @Bean
    public DynamoDbTable<Trade> getTradeTable() throws DynamoDBConnectionException {
        DynamoDbTable<Trade> table;
        try {
            DynamoDbEnhancedClient ddbClient = DynamoDbEnhancedClient.builder()
                    .dynamoDbClient(this.getDynamoDbClient())
                    .build();

            table = ddbClient.table(awsProperties.getStateTableName(), TableSchema.fromClass(Trade.class));
        } catch (Exception e) {
            log.error("Error Connecting to dynamoDB or getting Table", e);
            throw new DynamoDBConnectionException("Error Connecting to DynamoDB", e);
        }
    ...
    @Bean
    public void settlementKclScheduler() throws KinesisStreamException {
        Region region = Region.of(awsProperties.getRegion());
        String streamName = awsProperties.getSettlementInboundStream();
        SettlementRecordProcessorFactory settlementRecordProcessorFactory = new SettlementRecordProcessorFactory();

        ConfigsBuilder configsBuilder = getConfigBuilder(streamName, region, settlementRecordProcessorFactory);
        RetrievalConfig retrievalConfig = getRetrievalConfig(configsBuilder, streamName);
