...
resource "aws_codecommit_repository" "source_repo" {
  ...
  description     = "This is a source code repository in the cloud"
}
  ...
  event_pattern = <<PATTERN
{
  ...
  "detail": {
    ...
    "referenceName": [ "${var.REPOSITORY_BRANCH_NAME}" ]
  }
}
PATTERN
...
resource "aws_cloudwatch_event_target" "target_pipeline" {
  ...
  role_arn  = aws_iam_role.trigger_role.arn
  target_id = "${var.REPOSITORY_NAME}-${var.REPOSITORY_BRANCH_NAME}-pipeline"
}
